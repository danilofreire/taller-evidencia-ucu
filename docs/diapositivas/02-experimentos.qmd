...
subtitle: "Bloque 1: experimentos"
author:
  - name: Danilo Freire
...
format:
  clean-revealjs:
    self-contained: true
    footer: "[Bloque 1](https://danilofreire.github.io/taller-evidencia-ucu/diapositivas/02-experimentos.html)"
    transition: slide
...
:::{.columns}
:::{.column width=54%}
Si el tratamiento se asigna [al azar]{.alert}, recibirlo deja de depender de los resultados potenciales

En promedio, los grupos quedan [balanceados]{.alert}

- La condición que lo hace funcionar: $E[Y_{0,i}\mid T_i=1] = E[Y_{0,i}\mid T_i=0]$

- Con eso el sesgo se anula [en expectativa]{.alert}, y la diferencia de medias ya es el efecto causal promedio
...
:::{.column width=46%}
```{r sortear-fig, fig.width=4.6, fig.height=3.1}
set.seed(2026)
n_demo <- 400
...
Bajo sorteo, las dos distribuciones se superponen: no hay brecha previa que confundir con el efecto
:::
:::
:::
:::

## Por qué nos gustan los experimentos

:::{style="margin-top: 6px; font-size: 24px;"}
:::{.columns}
:::{.column width=50%}
Más allá de quitar el sesgo, el sorteo trae ventajas [poco evidentes]{.alert}:

- [Estimación simple]{.alert}: el efecto es una resta de medias, idéntica al coeficiente de una regresión

- [Transparencia]{.alert}: el análisis se promete de antemano, sin margen para elegir el resultado que gusta

- [Credibilidad]{.alert}: "sorteamos quién recibía" lo entiende un ministro o un juez, sin ecuaciones

- [Equidad]{.alert}: ante recursos escasos, todos tienen la misma chance de entrar

- Equilibra hasta las variables [que no se te ocurrió medir]{.alert}
:::

:::{.column width=50%}
```{r prepost-fig, fig.width=4.7, fig.height=3.3}
pp <- data.frame(
...
El antes-después del tratado sugiere $+6$. Pero el control [también sube]{.alert} ($+3$ por pura tendencia). El efecto es la brecha final, $16-13$: el control sorteado separa el efecto de la tendencia
:::
:::
:::
:::
...
:::{.columns}
:::{.column width=56%}
Un [experimento de campo]{.alert} aleatorizado, 196 agentes que trabajan de a pares sobre 139 sectores censales, todo en [un solo día]{.alert} (5 de mayo de 2018).

- [Reclutamiento]{.alert} por Facebook → formulario → capacitación (biología del vector y protocolo). Quien no fue a la capacitación quedó afuera

- [Sorteo simple]{.alert} a tres grupos, verificado con pruebas de balance sobre siete variables previas 
  - Edad, altruismo, religiosidad, ingreso, compromiso político y social, popularidad en Facebook

- Cada par recibía un mapa con una ruta de unas [120 casas]{.alert} en dos o tres barrios

- La productividad se [verifica con evidencia]{.alert}: foto del criadero eliminado, video de las larvas, foto de la casa. No con lo que el agente dice
:::
...
:::{.columns}
:::{.column width=56%}
Los agentes se sortearon a [tres grupos]{.alert}, citados en sedes distintas para evitar contagios:
...
:::{.column width=44%}
:::{style="text-align: center; margin-top: 6px;"}
[![](figures/dengue-uniforme.jpeg){width="75%"}](#){data-modal-type="image" data-modal-url="figures/dengue-uniforme.jpeg"}
:::

:::{style="font-size: 20px; text-align: center; color: #555;"}
Equipos de campo, con uniforme e identificación
:::
:::
:::
:::
...
- Un coordinador movió pares cerca de su base, y desbalanceó la asignación territorial
...
## Antes de correr al campo

:::{style="margin-top: 6px; font-size: 23px;"}
:::{.columns}
:::{.column width=56%}
Lo que dirían Duflo, Banerjee, Gerber o Green antes de arrancar:

- [Elegí bien el resultado]{.alert}: productividad y enfermedad no son lo mismo, y deciden qué vas a poder concluir

- [Hacé un piloto]{.alert}: probá el protocolo con pocos casos antes de escalar

- Calculá el [poder]{.alert} de antemano: acá se estimó asumiendo un efecto de 0,15 desvíos estándar

- Escribí un [plan de análisis previo]{.alert} y registralo antes de ver los datos ([Olken, 2015](https://doi.org/10.1257/jep.29.3.61))

- [Medí de forma objetiva]{.alert} y anticipá la atrición y los contagios
 
- Un experimento en zonas pobres es caro, lento y frágil. La mitad del rigor se juega [antes]{.alert} de recoger los datos
:::

:::{.column width=44%}
:::{style="text-align: center;"}
[![](figures/dengue-campo.jpeg){width="70%"}](#){data-modal-type="image" data-modal-url="figures/dengue-campo.jpeg"}
:::

:::{style="font-size: 20px; text-align: center; color: #555;"}
Trabajo de campo, casa por casa
:::
:::
:::
:::
...
## Generá los datos de la práctica
