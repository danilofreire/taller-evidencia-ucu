---
title: "Políticas públicas basadas en evidencia"
subtitle: "Parte 3: regresión discontinua"
author:
  - name: Danilo Freire
    orcid: 0000-0002-4712-6810
    email: danilofreire@gmail.com
    affiliations: "Department of Data and Decision Sciences <br> Emory University"
format:
  clean-revealjs:
    self-contained: true
    footer: "[RDD](https://danilofreire.github.io/taller-evidencia-ucu/diapositivas/03-rdd.html)"
    transition: slide
    transition-speed: default
    scrollable: true
revealjs-plugins:
  - multimodal
engine: knitr
editor:
  render-on-save: true
lang: es
---

```{r setup, include=FALSE}
options(htmltools.dir.version = FALSE)
library(knitr)
opts_chunk$set(
  echo = FALSE,
  fig.align = "center",
  dpi = 300,
  cache = FALSE
)

options(repos = c(CRAN = "https://cran.rstudio.com/"))

ensure <- function(pkg) {
  if (!require(pkg, character.only = TRUE)) {
    install.packages(pkg, dependencies = TRUE)
    library(pkg, character.only = TRUE)
  }
}
invisible(lapply(c("ggplot2", "dplyr", "estimatr", "rdrobust", "rddensity",
                   "fabricatr"), ensure))

suppressPackageStartupMessages({
  library(ggplot2); library(dplyr); library(estimatr)
  library(rdrobust); library(rddensity); library(fabricatr)
})

# paleta de la casa (mismo par azul/rojo que 01-apertura.qmd)
azul <- "#2d4563"; rojo <- "#b85450"

tema_taller <- theme_minimal(base_size = 15) +
  theme(panel.grid.minor = element_blank(),
        plot.margin = margin(6, 10, 6, 6))

# los datos del caso: simulados con fabricatr, el MISMO generador que se
# muestra en la slide "Generá los datos del caso".
# Semilla fija => reproduce exactamente datos/municipios.csv.
set.seed(148)
datos <- fabricate(
  N = 2000,
  poblacion      = round(runif(N, 20000, 75000)),
  poblacion_c    = poblacion - 47619,
  x              = poblacion_c / 10000,
  concejo_grande = as.integer(poblacion_c >= 0),
  n_concejales   = 9 + concejo_grande,
  pib_pc         = round(12 + 0.8 * x + rnorm(N, 0, 2), 1),
  prop_pobres    = round(pmin(pmax(0.34 - 0.02 * x + rnorm(N, 0, 0.05), 0.05), 0.75), 3),
  bancas_2000    = 9 + rbinom(N, 1, 0.15),
  mortalidad_infantil     = pmax(2,   round(20 - 0.5 * x - 2.01 * concejo_grande + rnorm(N, 0, 2.5), 1)),
  mortalidad_postneonatal = pmax(0.5, round(7 - 0.2 * x - 0.9 * concejo_grande + rnorm(N, 0, 1.3), 1)),
  matricula_primaria      = pmax(0,   round(24 + 0.4 * x + 2.58 * concejo_grande + rnorm(N, 0, 3), 1)),
  ideb           = pmin(10, pmax(0, round(4.5 + 0.1 * x + rnorm(N, 0, 0.5), 1))),
  burocratas     = pmax(0, round(480 + 12 * x + 104 * concejo_grande + rnorm(N, 0, 45))),
  proyectos_servicios = pmax(0, round(40 + 2 * x + 6 * concejo_grande + rnorm(N, 0, 7)))
)
datos$municipio <- sprintf("M%04d", seq_len(nrow(datos)))
datos <- datos[, c("municipio", "poblacion", "poblacion_c", "concejo_grande",
                   "n_concejales", "pib_pc", "prop_pobres", "bancas_2000",
                   "mortalidad_infantil", "mortalidad_postneonatal",
                   "matricula_primaria", "ideb", "burocratas",
                   "proyectos_servicios")]
```

# Regresión discontinua: el contrafactual que encontrás {background-color="#2d4563"}

## El plan del bloque

:::{style="margin-top: 10px; font-size: 24px;"}
:::{.columns}
:::{.column width=50%}
[Las ideas]{.alert}

- La [aleatorización local]{.alert} y el contrafactual que se [encuentra]{.alert}, no se construye
- El efecto [local]{.alert} (LATE) y a quién le aplica de verdad
- Los tres supuestos y sus [chequeos]{.alert}: densidad, balance, placebo
- Qué cambia cuando hay [muchos umbrales]{.alert} en vez de uno
- El [mecanismo político]{.alert} detrás del efecto sobre salud y educación
:::

:::{.column width=50%}
[Las herramientas]{.alert}

- `fabricatr` para [generar]{.alert} los datos del caso, como a la mañana
- `rdplot` para ver el [salto]{.alert} antes de estimar
- `rdrobust` para estimarlo con [ancho de banda óptimo]{.alert}
- `rddensity` para testear [manipulación]{.alert}
- Los chequeos como [rutina]{.alert}
:::
:::

:::{style="margin-top: 15px; border-left: 4px solid #2d4563; padding: 6px 18px; font-size: 23px;"}
Un caso de nuestra propia [investigación en la región]{.alert}: concejos municipales y bienestar en Brasil ([Mignozzetti, Cepaluni y Freire, 2025](https://doi.org/10.1111/ajps.12843), *AJPS*), con la regla del TSE de 2004 como [experimento accidental]{.alert}, y una práctica guiada donde generás los datos y corrés el código
:::
:::

## Antes de empezar: los paquetes

:::{style="margin-top: 16px; font-size: 23px;"}
Vas a necesitar cinco paquetes. Instalalos ahora, mientras arrancamos.

```{r instalar, echo=TRUE, eval=FALSE}
# Instalar (solo la primera vez)
install.packages(c("tidyverse", "estimatr", "rdrobust", "rddensity", "fabricatr"))

# Los datos los generamos nosotros en un rato, con fabricatr.
# ¿Preferís bajar el CSV ya armado? Leélo directo desde la web:
# datos <- read.csv("https://raw.githubusercontent.com/danilofreire/taller-evidencia-ucu/main/diapositivas/datos/municipios.csv")
```

:::{style="margin-top: 40px; border-left: 4px solid #b85450; padding: 6px 18px; font-size: 22px;"}
`estimatr` da errores robustos; `rdrobust` y `rddensity` son el estándar para RDD hoy; `fabricatr` ya lo conocés de la mañana. La línea comentada te evita generar los datos a mano
:::
:::

# La historia: un resultado que costó creer {background-color="#2d4563"}

## El caso: concejos más grandes en Brasil

:::{style="margin-top: 10px; font-size: 22px;"}
:::{.columns}
:::{.column width=52%}
Pregunta: ¿el [tamaño del concejo municipal]{.alert} afecta el bienestar de la gente?

- Hasta 2004 cada municipio fijaba su propio concejo

- El [desbalance]{.alert} era enorme: Nova Russas, un concejal cada 1.429 habitantes; Sorocaba, uno cada 35.249

- Todo empezó con [Mira Estrela]{.alert}, un pueblo que quiso recortar su concejo de 11 a 9 bancas; el caso llegó al Supremo

- La respuesta del TSE, en 2004: [tramos de población]{.alert}. 9 bancas, más una cada 47.619 habitantes, hasta 21

- Los municipios [no eligen su población]{.alert}: comparás los que quedaron [apenas arriba y apenas abajo]{.alert} de un corte ([Mignozzetti, Cepaluni y Freire, 2025](https://doi.org/10.1111/ajps.12843))
:::

:::{.column width=48%}
:::{style="text-align: center;"}
[![](figures/rdd-primera-pagina.png){width="72%"}](#){data-modal-type="image" data-modal-url="figures/rdd-primera-pagina.png"}
:::
:::{style="text-align: center; font-size: 18px; color: #555;"}
Mignozzetti, Cepaluni y Freire (2025), *American Journal of Political Science*
:::
:::
:::
:::

## La regla que crea el experimento

:::{style="margin-top: 6px; font-size: 22px;"}
:::{.columns}
:::{.column width=52%}
La ley funciona como el [mecanismo de asignación]{.alert}: 9 concejales, más uno cada 47.619 habitantes, hasta 21. Son [12 umbrales]{.alert} donde el número de bancas salta de golpe

- La variable de asignación es la [proyección de población de 2003]{.alert} del IBGE, predeterminada: nadie puede acomodarse para caer de un lado o del otro

- La regla llegó [siete meses antes]{.alert} de la elección de octubre de 2004: sin tiempo para ajustar estrategias

- Los efectos se miden en el mandato [2005-2008]{.alert}, ya con los concejos nuevos funcionando

- Nadie diseñó un experimento: la [ley lo creó sin querer]{.alert}, y nosotros lo aprovechamos 😉
:::

:::{.column width=48%}
:::{style="text-align: center;"}
[![](figures/rdd-umbrales.png){width="100%"}](#){data-modal-type="image" data-modal-url="figures/rdd-umbrales.png"}
:::

:::{style="font-size: 18px; text-align: center; color: #555;"}
Cada escalón es un umbral: al cruzarlo, el concejo suma una banca
:::
:::
:::
:::

## Lo que encontramos

:::{style="margin-top: 6px; font-size: 22px;"}
:::{.columns}
:::{.column width=50%}
Por cada [concejal extra]{.alert}, midiendo apenas alrededor de cada umbral:

- [Mortalidad infantil]{.alert}: −2,01 por mil nacidos vivos (~0,14 desvíos estándar)

- [Mortalidad postneonatal]{.alert}: −0,90 por mil (~0,14 DE)

- [Matrícula primaria]{.alert}: +2,58 chicos por aula (~0,20 DE)

- La [calidad educativa]{.alert} (IDEB) no cae ni sube: −0,03, no significativo. Los placebos también dan nulos

- Detrás de estos números hay una [historia política]{.alert}, y la vemos en unos minutos
:::

:::{.column width=50%}
:::{style="text-align: center;"}
[![](figures/rdd-bienestar.png){width="100%"}](#){data-modal-type="image" data-modal-url="figures/rdd-bienestar.png"}
:::

:::{style="font-size: 18px; text-align: center; color: #555;"}
Estimados de bienestar por concejal extra, con sus placebos
:::
:::
:::

:::{style="margin-top: 6px; border-left: 4px solid #b85450; padding: 6px 18px; font-size: 20px;"}
Concejos más grandes, mejor salud y mejor educación... **¡en Brasil!** 😅 La reacción sana es [sospechar]{.alert}: ¿no eran esos municipios distintos de entrada (más poblados, más ricos)? Eso es [sesgo de selección]{.alert}
:::
:::

# La solución: regresión discontinua {background-color="#2d4563"}

## La idea: aleatorización local

:::{style="margin-top: 22px; font-size: 24px;"}
Cuando el tratamiento se decide por un [umbral]{.alert} en una variable continua, los que quedan [apenas por debajo]{.alert} y [apenas por encima]{.alert} son casi indistinguibles.

- Un municipio de 47.610 habitantes y otro de 47.630 se parecen en [todo]{.alert}, menos en que uno cruzó el corte y ganó una banca

- Cerca del umbral, recibir el tratamiento es [casi un sorteo]{.alert}

- Comparar justo ahí nos da un contrafactual creíble ([Lee y Lemieux, 2010](https://doi.org/10.1257/jel.48.2.281); [Cattaneo, Idrobo y Titiunik, 2020](https://doi.org/10.1017/9781108684606))

:::{style="margin-top: 30px; border-left: 4px solid #b85450; padding: 6px 18px; font-size: 23px;"}
Lo que estimás es un [efecto local promedio]{.alert}, el LATE. Vale para municipios pegados al corte
:::
:::

## La ecuación

:::{style="margin-top: 20px; font-size: 24px;"}
Con la población centrada ($X_i =$ población $- 47.619$) y $T_i = \mathbf{1}(X_i \ge 0)$ el indicador de haber cruzado el corte, estimamos:

:::{style="margin-top: 4px; font-size: 27px; text-align: center;"}
$$
Y_i = \beta_0 + \beta_1 X_i + \beta_2\, T_i + \beta_3\,(X_i \cdot T_i) + \varepsilon_i
$$
:::

- $X_i$ es la población [centrada]{.alert} en el umbral: el corte queda en $X_i = 0$

- $\beta_1 X_i$ y $\beta_3\,(X_i \cdot T_i)$ dejan que el resultado [varíe suave]{.alert} con la población, con una [pendiente a cada lado]{.alert} del corte (es lo que hace `rdrobust`)

- $T_i$ vale 1 arriba del corte; su coeficiente $\beta_2$ es [el salto]{.alert}, el [efecto local]{.alert} que buscamos

- Acá el diseño es [sharp]{.alert}: cruzar el corte determina el tratamiento sin excepciones

- Existen diseños [fuzzy]{.alert}, donde cruzar sólo cambia la probabilidad de tratarse; se estiman con una variante del mismo método

:::{style="margin-top: 8px; border-left: 4px solid #2d4563; padding: 6px 18px; font-size: 24px;"}
Supuesto clave: sin el tratamiento, el resultado habría seguido [la misma curva suave]{.alert} a través del corte. Todo salto abrupto es el tratamiento
:::
:::

## Generá los datos del caso

:::{style="margin-top: 4px; font-size: 22px;"}
Con `fabricatr` simulamos 2.000 municipios sobre el diseño del paper: un solo corte (47.619 habitantes, 9 → 10 bancas) y efectos calcados de la investigación real. Leamos el código juntos:

:::{.columns}
:::{.column width=55%}
:::{style="margin-top: 6px; font-size: 17px;"}
```{r gen-datos-show, echo=TRUE, eval=FALSE}
library(fabricatr)
set.seed(148)

datos <- fabricate(
  N = 2000,
  poblacion      = round(runif(N, 20000, 75000)),
  poblacion_c    = poblacion - 47619,           # población centrada en el umbral
  x              = poblacion_c / 10000,          # en decenas de miles
  concejo_grande = as.integer(poblacion_c >= 0), # cruzar el umbral asigna
  n_concejales   = 9 + concejo_grande,           # 9 → 10 bancas
  pib_pc         = round(12 + 0.8 * x + rnorm(N, 0, 2), 1),
  prop_pobres    = round(pmin(pmax(0.34 - 0.02 * x
                     + rnorm(N, 0, 0.05), 0.05), 0.75), 3),
  bancas_2000    = 9 + rbinom(N, 1, 0.15),
  mortalidad_infantil     = pmax(2,   round(20 - 0.5 * x
    - 2.01 * concejo_grande + rnorm(N, 0, 2.5), 1)),  # el salto del paper: −2,01
  mortalidad_postneonatal = pmax(0.5, round(7 - 0.2 * x
    - 0.9 * concejo_grande + rnorm(N, 0, 1.3), 1)),
  matricula_primaria      = pmax(0,   round(24 + 0.4 * x
    + 2.58 * concejo_grande + rnorm(N, 0, 3), 1)),
  ideb           = pmin(10, pmax(0, round(4.5 + 0.1 * x
    + rnorm(N, 0, 0.5), 1))),
  burocratas     = pmax(0, round(480 + 12 * x
    + 104 * concejo_grande + rnorm(N, 0, 45))),
  proyectos_servicios = pmax(0, round(40 + 2 * x
    + 6 * concejo_grande + rnorm(N, 0, 7)))
)
```
:::
:::

:::{.column width=45%}
:::{style="margin-top: 6px; font-size: 20px;"}
```{r gen-datos-head, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
datos |>
  select(poblacion_c, n_concejales,
         mortalidad_infantil) |>
  head()
```
:::

:::{style="margin-top: 6px; font-size: 22px; color: #555;"}
La línea que importa es `concejo_grande`: acá no hay sorteo, [asigna la ley]{.alert}. El resto son variables de resultado con el salto del paper cargado a mano
:::

:::{style="margin-top: 6px; font-size: 19px; color: #555;"}
El mismo código está en el script de la práctica; el CSV del repo es idéntico (misma semilla)
:::
:::
:::
:::

## "Gemelos" en el umbral

:::{style="margin-top: 14px; font-size: 24px;"}
Si la idea es cierta, arriba y abajo del corte los municipios tienen que ser [gemelos]{.alert} en todo lo que se decidió antes. Mirá los que caen a menos de 4.000 habitantes del umbral:

```{r gemelos, echo=TRUE}
datos |>
  filter(abs(poblacion_c) < 4000) |>
  group_by(concejo_grande) |>
  summarise(across(c(pib_pc, prop_pobres, bancas_2000, n_concejales), mean))
```

:::{style="margin-top: 30px; border-left: 4px solid #2d4563; padding: 6px 18px; font-size: 24px;"}
PIB, pobreza y bancas previas casi no se mueven entre los dos grupos. Lo único que cambia de golpe es [una banca]{.alert}: ese es el experimento que escribió la ley
:::
:::

## Ver el salto: a mano y con `rdplot`

:::{style="margin-top: 4px; font-size: 21px;"}
Antes de estimar nada, [mirá]{.alert} los datos: promedios por tramo, con la recta suave a cada lado. Lo que armás a mano, `rdplot` lo hace en una línea (elige los bins y ajusta el polinomio):

:::{.columns}
:::{.column width=50%}
```{r salto, echo=FALSE, fig.height=3.9, fig.width=5.2}
d <- datos
brks <- seq(-30000, 30000, by = 2500)
d$bin <- cut(d$poblacion_c, breaks = brks)
ctr <- (head(brks, -1) + tail(brks, -1)) / 2
agg <- data.frame(centro = ctr,
                  y = as.numeric(tapply(d$mortalidad_infantil, d$bin, mean)))
agg <- agg[!is.na(agg$y), ]

ggplot() +
  geom_point(data = agg, aes(centro, y), colour = azul, size = 2.4) +
  geom_smooth(data = subset(d, poblacion_c < 0),
              aes(poblacion_c, mortalidad_infantil),
              method = "lm", se = FALSE, colour = azul, linewidth = 1) +
  geom_smooth(data = subset(d, poblacion_c >= 0),
              aes(poblacion_c, mortalidad_infantil),
              method = "lm", se = FALSE, colour = azul, linewidth = 1) +
  geom_vline(xintercept = 0, linetype = "dashed", colour = rojo, linewidth = 0.8) +
  labs(x = "Población centrada en el umbral", y = "Mortalidad infantil (por mil)") +
  tema_taller
```

:::{style="text-align: center; font-size: 17px; color: #555;"}
A mano: promedios por tramo y una recta a cada lado
:::
:::

:::{.column width=50%}
```{r verrdplot, echo=TRUE, fig.height=3.9, fig.width=5.2}
rdplot(datos$mortalidad_infantil,
       datos$poblacion_c,
       x.label = "Población centrada",
       y.label = "Mortalidad infantil",
       title = "")
```
:::
:::

:::{style="text-align: center; font-size: 19px; color: #555;"}
El salto hacia abajo en el corte es el efecto; la pendiente es la tendencia que hay que descontar
:::
:::

## Comparación ingenua vs local

:::{style="margin-top: 14px; font-size: 24px;"}
La diferencia entre [todos]{.alert} los concejos grandes y chicos exagera el efecto, porque arrastra la tendencia:

```{r naive, echo=TRUE}
with(datos, mean(mortalidad_infantil[concejo_grande == 1]) -
            mean(mortalidad_infantil[concejo_grande == 0]))
```

La regresión [local]{.alert}, cerca del corte, descuenta esa pendiente:

```{r local, echo=TRUE}
cerca <- subset(datos, abs(poblacion_c) < 8000)
fit <- lm_robust(mortalidad_infantil ~ concejo_grande + poblacion_c +
                   concejo_grande:poblacion_c, data = cerca)
round(coef(summary(fit))["concejo_grande", 1:4], 3)
```

:::{style="margin-top: 4px; border-left: 4px solid #2d4563; padding: 6px 18px; font-size: 21px;"}
De [−3,57]{.alert} (ingenua) a [≈ −1,9]{.alert} (local): la brecha era sesgo de selección, no efecto
:::
:::

## Estimarlo bien: `rdrobust`

:::{style="margin-top: 12px; font-size: 19px;"}
`rdrobust` elige el ancho de banda de forma óptima y corrige el sesgo de los bordes ([Calonico, Cattaneo y Titiunik, 2014](https://doi.org/10.3982/ECTA11757)):

```{r rdrobust, echo=TRUE}
rd <- rdrobust(datos$mortalidad_infantil, datos$poblacion_c)
summary(rd)
```

:::{style="margin-top: 8px;"}
El efecto local es de [−1,92 muertes por mil]{.alert}. En el paper, con los datos reales y los 12 umbrales agrupados, da [−2,01]{.alert} (Tabla 4)
:::
:::

## El efecto en varias dimensiones

:::{style="margin-top: 10px; font-size: 25px;"}
El mismo salto, resultado por resultado:

```{r coda, echo=FALSE}
outs <- c(matricula_primaria = "Matrícula primaria (chicos/aula)",
          mortalidad_infantil = "Mortalidad infantil (por mil)",
          mortalidad_postneonatal = "Mortalidad postneonatal (por mil)",
          ideb = "Calidad educativa (IDEB, 0-10)",
          burocratas = "Cargos designados",
          proyectos_servicios = "Proyectos de servicios")
tab <- t(sapply(names(outs), function(v) {
  m <- rdrobust(datos[[v]], datos$poblacion_c)
  c(Efecto = round(m$coef[1], 2), p = round(m$pv[1], 3))
}))
rownames(tab) <- outs
knitr::kable(tab)
```

:::{style="margin-top: 2px; border-left: 4px solid #b85450; padding: 6px 18px; font-size: 21px;"}
Mejor salud, más matrícula, más cargos y proyectos. La [calidad (IDEB) no salta]{.alert}: entran más chicos al aula sin que el nivel medido caiga, igual que en el paper (−0,03, no significativo). Un nulo acá es [información]{.alert}, no un fracaso
:::
:::

# ¿Por qué un concejal más mejora el bienestar? {background-color="#2d4563"}

## El mecanismo: de la banca al bienestar

:::{style="margin-top: 6px; font-size: 22px;"}
:::{.columns}
:::{.column width=52%}
¿Por qué una banca más mejora el bienestar? La cadena es [política]{.alert}:

- Un concejal extra tiene [91%]{.alert} de chance de sumarse a la coalición del intendente: la banca marginal casi siempre [apoya al oficialismo]{.alert}

- Ese apoyo se compra con [cargos]{.alert}: +104 burócratas designados por banca (los de [carrera]{.alert} y los asistentes no se mueven, un placebo interno)

- Con la coalición asegurada, el concejo aprueba [más leyes de servicios]{.alert}: +15,0% sobre bienes públicos, +19,7% de salud y educación

- [Coalición]{.alert} → [designados]{.alert} → [leyes de servicios]{.alert} → mejor [salud y educación]{.alert}
:::

:::{.column width=48%}
:::{style="text-align: center;"}
[![](figures/rdd-tabla3-mecanismo.png){width="100%"}](#){data-modal-type="image" data-modal-url="figures/rdd-tabla3-mecanismo.png"}
:::

:::{style="font-size: 20px; text-align: center; color: #555;"}
Tabla 3 del paper: coalición y designados saltan; carrera y asistentes, no. Una encuesta a 174 ex concejales [triangula]{.alert} el mecanismo
:::
:::
:::
:::

# Hacerlo creíble: validez y pruebas {background-color="#2d4563"}

## Tres supuestos

:::{style="margin-top: 22px; font-size: 25px;"}
El RDD del paper descansa en tres supuestos, y cada uno tiene su prueba:

1. [Nadie manipula el umbral]{.alert}: los municipios no pueden elegir de qué lado caer

2. [El salto es nítido]{.alert}: el concejo crece exactamente como manda la ley (RDD sharp)

3. [Las covariables previas no saltan]{.alert}: medidas antes de 2004, no deberían cambiar en el corte

Y una pregunta que le hacés [siempre]{.alert} a tu RDD: ¿cambia [otra política]{.alert} en el mismo umbral (transferencias, salarios, categorías administrativas)? Si la respuesta es sí, el salto mezcla los dos tratamientos y ya no medís sólo la banca.

:::{style="margin-top: 50px; border-left: 4px solid #2d4563; padding: 6px 18px; font-size: 24px;"}
Un supuesto no se declara: se [pone a prueba]{.alert}. Vamos una por una, y después rompemos el diseño a propósito
:::
:::

## Chequeo 1: ¿manipularon el umbral?

:::{style="margin-top: 8px; font-size: 21px;"}
Si los municipios se acomodaran para cruzar el corte, se [amontonarían]{.alert} de un lado. El test de densidad busca ese amontonamiento ([Cattaneo, Jansson y Ma, 2020](https://doi.org/10.1080/01621459.2019.1635480)):

:::{.columns}
:::{.column width=50%}
```{r densidad, echo=TRUE}
dens <- rddensity(datos$poblacion_c)
round(c(T = dens$test$t_jk,
        p = dens$test$p_jk), 3)
```

$p = 0{,}709$: [no hay salto]{.alert} en la densidad. La población de 2003 no se podía manipular, tal como esperábamos
:::

:::{.column width=50%}
Para creerle a un chequeo, hay que verlo [fallar]{.alert} cuando debe. Rompemos el diseño a propósito: empujamos al 70% de los que estaban justo por debajo a cruzar el corte

```{r sabotaje, echo=TRUE}
set.seed(1)
malo <- datos
borde <- malo$poblacion_c > -1500 &
         malo$poblacion_c < 0
mover <- borde & runif(nrow(malo)) < 0.7
malo$poblacion_c[mover] <-
  abs(malo$poblacion_c[mover]) + 200

round(rddensity(malo$poblacion_c)$test$p_jk, 4)
```
:::
:::

:::{style="margin-top: 4px; border-left: 4px solid #b85450; padding: 6px 18px; font-size: 20px;"}
De $p = 0{,}71$ a $p \approx 0{,}002$: el test [detecta]{.alert} la manipulación que fabricamos. Por eso vale la pena correrlo, y por eso su silencio sobre los datos reales [informa]{.alert}
:::
:::

## Chequeo 2: balance en covariables

:::{style="margin-top: 14px; font-size: 23px;"}
Las variables medidas [antes]{.alert} del tratamiento (PIB per cápita, pobreza) no deberían saltar en el umbral. Corremos el RDD sobre ellas como [placebo]{.alert}:

```{r balance, echo=TRUE}
# 1. un RDD por covariable: ¿salta en el corte?
b_pib    <- rdrobust(datos$pib_pc, datos$poblacion_c)
b_pobres <- rdrobust(datos$prop_pobres, datos$poblacion_c)

# 2. juntamos el salto y su valor p en una tabla
balance <- data.frame(
  variable = c("pib_pc", "prop_pobres"),
  salto    = round(c(b_pib$coef[1], b_pobres$coef[1]), 3),
  p_valor  = round(c(b_pib$pv[1], b_pobres$pv[1]), 3)
)
balance
```

:::{style="margin-top: 8px;"}
Los dos $p$ son [altos]{.alert}: ningún salto donde no debería haberlo. Los grupos son comparables de entrada
:::

:::{style="margin-top: 6px; font-size: 22px; color: #555;"}
En el paper: bancas del período previo, población, PIB y familias de bajos ingresos (datos del TSE y del censo 2000) no saltan en el corte (Tabla 2)
:::
:::

## Chequeo 3: un corte placebo

:::{style="margin-top: 14px; font-size: 25px;"}
Si inventamos un umbral [falso]{.alert} donde la ley no cambia nada, no debería aparecer ningún efecto:

```{r placebo, echo=TRUE}
rdrobust(datos$mortalidad_infantil, datos$poblacion_c,
         c = -15000)$pv[1]
```

:::{style="margin-top: 8px;"}
$p \approx 0{,}88$: [nada]{.alert} en el corte inventado. El salto aparece sólo en el umbral verdadero
:::

:::{style="margin-top: 30px; border-left: 4px solid #2d4563; padding: 6px 18px; font-size: 23px; margin-bottom: 30px;"}
Un efecto que aparece en umbrales placebo es una [señal de alarma]{.alert}: probablemente estás capturando la tendencia, no un salto
:::

:::{style="margin-top: 6px; font-size: 22px; color: #555;"}
En el paper: se ajustaron cortes falsos entre los umbrales reales; ninguno muestra efecto
:::
:::

## Chequeo 4: ¿depende del ancho de banda?

:::{style="margin-top: 2px; font-size: 23px;"}
Un resultado creíble no cambia de signo ni de tamaño según cuán ancha sea la ventana:

```{r ancho, echo=FALSE, fig.height=3.9, fig.width=12}
hs <- seq(4000, 16000, by = 2000)
est <- sapply(hs, function(h) {
  m <- rdrobust(datos$mortalidad_infantil, datos$poblacion_c, h = h)
  c(m$coef[1], m$se[1])
})
bw <- data.frame(h = hs, tau = est[1, ], se = est[2, ])

ggplot(bw, aes(h, tau)) +
  geom_hline(yintercept = 0, colour = "grey70") +
  geom_ribbon(aes(ymin = tau - 1.96 * se, ymax = tau + 1.96 * se),
              fill = azul, alpha = 0.15) +
  geom_line(colour = azul, linewidth = 1) +
  geom_point(colour = azul, size = 2.4) +
  labs(x = "Ancho de banda (habitantes)", y = "Efecto estimado") +
  tema_taller
```

:::{style="text-align: center; font-size: 20px; color: #555;"}
El efecto se queda estable en torno a −1,9, ventana tras ventana
:::

:::{style="margin-top: 6px; font-size: 19px; color: #555;"}
En el paper: el ancho lo elige el método de Calonico, Cattaneo y Titiunik (2014), y los resultados aguantan variarlo
:::
:::

# Hacerlo en la región {background-color="#2d4563"}

## Cómo encontrar un buen RDD (y sus datos)

:::{style="margin-top: 8px; font-size: 21px;"}
Un RDD vive donde una regla [asigna algo por un umbral]{.alert}, y la región está llena de reglas así, con microdata abierta para estudiarlas:

:::{.columns}
:::{.column width=50%}
[Reglas con umbral]{.alert}

- [Puntajes de focalización]{.alert}: SISBEN (Colombia), Cadastro Único (Brasil), Ficha CAS (Chile)

- [Notas de corte]{.alert}: becas y admisión, como Ser Pilo Paga ([Londoño-Vélez et al., 2020](https://doi.org/10.1257/pol.20180131))

- [Edad de elegibilidad]{.alert}: jubilaciones, pensiones, entrada a la escuela

- [Elecciones cerradas]{.alert}: ganar por poco reparte cargos casi al azar
:::

:::{.column width=50%}
[Dónde están los datos]{.alert}

- [Salud]{.alert}: DataSUS (Brasil) y los observatorios de salud

- [Censos y hogares]{.alert}: IBGE, INE; IPUMS los deja comparables

- [Electorales]{.alert}: TSE y las autoridades electorales, por sección

- [Multilaterales]{.alert}: BID, CAF, Banco Mundial Microdata
:::
:::

:::{style="margin-top: 6px; border-left: 4px solid #b85450; padding: 6px 18px; font-size: 20px;"}
Regla práctica: [buscá primero la ley]{.alert} que crea el corte, y pedí la variable de asignación con la [mayor resolución posible]{.alert} (el puntaje exacto, la población exacta). Sin eso, no hay salto que mirar
:::
:::

# Práctica {background-color="#2d4563"}

## De qué se trata la práctica {#sec:ejercicios}

:::{style="margin-top: 6px; font-size: 22px;"}
:::{.columns}
:::{.column width=56%}
Ahora te toca a vos: con los datos que [generaste]{.alert}, vas a estimar un RDD sobre [otra variable]{.alert} y a ponerlo a prueba.

Objetivos:

- ver el [salto]{.alert} con un gráfico simple
- estimar el [efecto local]{.alert} con `rdrobust`
- correr los [chequeos de validez]{.alert} (densidad, placebo, balance) como rutina
- comparar la estimación [ingenua]{.alert} con la local y ver cuánto cambia
:::

:::{.column width=44%}
[Las variables]{.alert}

:::{style="font-size: 20px;"}
| Variable | Qué es |
|:--|:--|
| `poblacion` | Proyección de población 2003 |
| `poblacion_c` | Población menos 47.619 |
| `concejo_grande` | 1 si cruzó el umbral |
| `n_concejales` | 9 o 10 bancas |
| `mortalidad_infantil` | Muertes por mil nacidos |
| `matricula_primaria` | Chicos por aula |
| `pib_pc`, `prop_pobres` | Covariables previas al corte |
:::

Usamos los datos que creamos, pero también podés bajarlos desde [la página del taller](https://danilofreire.github.io/taller-evidencia-ucu/materiales.html)
:::
:::

:::{style="margin-top: 6px; border-left: 4px solid #2d4563; padding: 6px 18px; font-size: 20px;"}
Tenés unos [20 minutos]{.alert}. Cada ejercicio tiene su [solución en el apéndice]{.alert}: intentalo primero, después revisá. No hace falta terminar los tres; lo importante es discutir lo que encontrás
:::
:::

## Ejercicio 1: estimá otro resultado {#sec:ej1}

:::{style="margin-top: 16px; font-size: 22px;"}
**Tarea:** repetí el análisis, pero sobre la [matrícula primaria]{.alert} (`matricula_primaria`) en lugar de la mortalidad. Hacé el gráfico del salto y estimá el efecto con `rdrobust`.

```{r}
#| label: ej1-skeleton
#| eval: false
#| echo: true
# 1. Ver el salto: primero a mano (un scatter con el corte marcado)...
plot(___, ___)                 # TODO: matricula_primaria contra poblacion_c
abline(v = 0, col = "red")

# ...y después con la herramienta estándar
___(datos$___, datos$___)      # TODO: ¿qué función dibuja el RDD?

# 2. Estimar el efecto local
rd <- ___(datos$___, datos$___)        # TODO: ¿y cuál lo estima?
round(c(efecto = rd$coef[1], p = rd$pv[1]), 3)
```

:::{.callout-note appearance="simple" icon=false}
**Pista**: la variable de asignación es siempre `poblacion_c` (población centrada en el corte). ¿El signo del efecto tiene sentido?
:::

[[Ver solución](#sec:sol-ej1)]{.button}
:::

## Ejercicio 2: poné el diseño a prueba {#sec:ej2}

:::{style="margin-top: 16px; font-size: 22px;"}
**Tarea:** ¿le creerías a tu estimación del Ejercicio 1? Corré los mismos chequeos que vimos: densidad, un corte placebo y balance en una covariable. Esta vez las funciones las ponés vos.

```{r}
#| label: ej2-skeleton
#| eval: false
#| echo: true
# a) manipulación: ¿qué test busca amontonamiento en el corte?
___(datos$___)$test$p_jk

# b) corte placebo: un umbral falso donde no debería pasar nada
___(datos$matricula_primaria, datos$poblacion_c, c = ___)$pv[1]

# c) balance: una covariable previa no debería saltar
___(datos$___, datos$poblacion_c)$pv[1]
```

:::{.callout-note appearance="simple" icon=false}
**Pista**: son las dos funciones que ya usaste hoy; para el placebo, probá un corte donde la ley no cambia nada (por ejemplo, 12000). ¿Cuál de los tres chequeos te preocuparía más si fallara?
:::

[[Ver solución](#sec:sol-ej2)]{.button}
:::

## Ejercicio 3: ingenua vs local, y a discutir {#sec:ej3}

:::{style="margin-top: 16px; font-size: 22px;"}
**Tarea:** compará la diferencia [ingenua]{.alert} (todos los grandes vs todos los chicos) con la estimación [local]{.alert} del RDD, para `matricula_primaria`. Escribí la regresión local vos, como hicimos con la mortalidad. Después, discutí con quien tengas al lado.

```{r}
#| label: ej3-skeleton
#| eval: false
#| echo: true
# diferencia ingenua
with(datos, mean(matricula_primaria[concejo_grande == 1]) -
            mean(matricula_primaria[concejo_grande == 0]))

# regresión local a mano: sólo cerca del corte, con la pendiente
# a cada lado (¿qué operador pide pendientes distintas?)
cerca <- filter(datos, abs(poblacion_c) < 8000)
lm_robust(matricula_primaria ~ concejo_grande ___ poblacion_c,
          data = cerca)

# efecto local con la herramienta estándar
___(datos$matricula_primaria, datos$poblacion_c)$coef[1]
```

:::{.callout-note appearance="simple" icon=false}
**Para discutir**: ¿por qué difieren la ingenua y la local? ¿Cuál reportarías y por qué? ¿Qué covariable podría explicar la brecha?
:::

[[Ver solución](#sec:sol-ej3)]{.button}
:::

## Para llevarte

:::{style="margin-top: 8px; font-size: 22px;"}
:::{.columns}
:::{.column width=50%}
[Las ideas]{.alert}

- El RDD [encuentra]{.alert} el contrafactual en una regla que ya estaba escrita

- Cerca del corte, caer de un lado o del otro es casi un [sorteo]{.alert}

- Acá no sortea nadie: [la regla asigna]{.alert}, y el diseño vive en la ley

- El efecto es [local]{.alert} (LATE): vale en el umbral, no en todos lados

- La credibilidad vive en los [chequeos]{.alert} (densidad, balance, placebo, ancho de banda), no en el estimador
:::

:::{.column width=50%}
[La caja de herramientas]{.alert}

- `fabricatr` para [generar]{.alert} los datos del caso, con la regla adentro

- `rdplot` para [mirar]{.alert} antes de estimar

- `rdrobust` con [ancho de banda óptimo]{.alert}

- `rddensity` para testear [manipulación]{.alert}

- El sabotaje de los datos como prueba de que los chequeos [detectan]{.alert} lo que tienen que detectar
:::
:::

:::{style="margin-top: 8px; border-left: 4px solid #2d4563; padding: 6px 18px; font-size: 21px;"}
Ya sabés [construir]{.alert} un contrafactual (sortear) y [encontrarlo]{.alert} (un umbral); a la tarde vemos qué hacer cuando no hay ninguno de los dos, y cómo se [acumula]{.alert} la evidencia entre estudios
:::
:::

## Para seguir

:::{style="margin-top: 8px; font-size: 20px;"}
:::{.columns}
:::{.column width=50%}
[Para leer]{.alert}

- [Cattaneo, Idrobo y Titiunik (2020)](https://doi.org/10.1017/9781108684606), *A Practical Introduction to RD Designs: Foundations*, y su segundo volumen, [*Extensions* (2024)](https://doi.org/10.1017/9781009441896): gratis online, la guía moderna

- [Lee y Lemieux (2010)](https://doi.org/10.1257/jel.48.2.281), el survey clásico en el *Journal of Economic Literature*

- [Angrist y Pischke (2015)](https://press.princeton.edu/books/paperback/9780691152844/mastering-metrics), *Mastering 'Metrics*; los capítulos de RDD de [*The Effect*](https://theeffectbook.net/) (Huntington-Klein, 2021) y de [*The Mixtape*](https://mixtape.scunning.com/) (Cunningham, 2021), gratis online

[Nuestro caso de hoy]{.alert}

- [Mignozzetti, Cepaluni y Freire (2025)](https://doi.org/10.1111/ajps.12843): concejos y bienestar

- Fichas completas en las [referencias](#sec:referencias)
:::

:::{.column width=50%}
[Herramientas en R]{.alert}

- [rdpackages.github.io](https://rdpackages.github.io/), el hub de los paquetes RDD de Cattaneo y coautores

- [`rdrobust`](https://rdpackages.github.io/rdrobust/): estimación, inferencia y `rdplot`

- [`rddensity`](https://rdpackages.github.io/rddensity/): tests de manipulación

- [`rdlocrand`](https://rdpackages.github.io/rdlocrand/): inferencia de aleatorización local en ventanas chicas

- [`rdmulti`](https://rdpackages.github.io/rdmulti/): varios cortes a la vez, como los 12 umbrales del paper

- [`rdpower`](https://rdpackages.github.io/rdpower/): poder y tamaño de muestra para RDD

- `regression_discontinuity_designer`, de la `DesignLibrary` que viste a la mañana: simulá tu RDD antes de correrlo
:::
:::
:::

# Cierre de la mañana {background-color="#2d4563"}

## Dos contrafactuales, una pregunta

:::{style="margin-top: 10px; font-size: 24px;"}
Toda la mañana giró alrededor de una sola pregunta: [comparado con qué]{.alert}. El experimento y el RDD la responden distinto, pero apuntan al mismo lugar.

:::{style="margin-top: 30px; margin-bottom: 30px;"}
| | [Experimento]{.alert} | [RDD]{.alert} |
|---|---|---|
| ¿De dónde sale el contrafactual? | Lo [construís]{.alert} sorteando | Lo [encontrás]{.alert} en una regla que ya existe |
| ¿Qué supuesto lo sostiene? | El sorteo salió bien: [balance]{.alert} | Nada más salta en el umbral: [continuidad]{.alert} |
| ¿Qué estima? | ATE en la muestra experimental | LATE [en el umbral]{.alert} |
| ¿Cuándo lo podés usar? | Cuando podés [asignar]{.alert} el tratamiento | Cuando una norma ya asignó por [corte]{.alert} |
:::

:::{style="margin-top: 10px; border-left: 4px solid #2d4563; padding: 6px 18px; font-size: 24px;"}
Cambian las herramientas y cambia el supuesto, pero la pregunta es siempre la misma: ¿comparado con qué?
:::
:::

## Elegí tu diseño

:::{style="margin-top: 12px; font-size: 25px;"}
Cinco minutos, en parejas. Para cada escenario: ¿[experimento]{.alert}, [RDD]{.alert}, o ninguno de los dos? ¿Y cuál es la [mayor amenaza]{.alert} a la validez?

:::{style="margin-top: 30px; border-left: 4px solid #b85450; padding: 8px 20px; font-size: 24px; margin-bottom: 30px;"}
a) El MEC quiere evaluar una [beca]{.alert} nueva que se otorga a los estudiantes que superan un [puntaje de corte]{.alert} en una prueba estandarizada de egreso

b) Una intendencia va a lanzar una [app de reclamos viales]{.alert} y puede elegir en qué [barrios]{.alert} activarla primero

c) Una [ley nacional]{.alert} de etiquetado frontal de alimentos entró en vigor el mismo día en [todo el país]{.alert}
:::
:::

## Para discutir

:::{style="margin-top: 12px; font-size: 24px;"}
Cinco minutos más, de a dos o tres, sobre estas dos consignas:

:::{style="margin-top: 40px; border-left: 4px solid #b85450; padding: 8px 20px; font-size: 24px; margin-bottom: 30px;"}
a) Te ofrecen dos evaluaciones del mismo programa: un [experimento]{.alert} con 200 personas o un [RDD]{.alert} con 20.000. ¿Cuál preferís y qué le preguntás a cada uno antes de creerle?

b) El RDD de los concejos vale cerca de [47.619]{.alert} habitantes. ¿Qué le decís al intendente de una ciudad de 300.000 que te pregunta si le conviene un [concejo más grande]{.alert}?
:::
:::

## Cuando no hay umbral ni sorteo

:::{style="margin-top: 10px; font-size: 24px;"}
:::{.columns}
:::{.column width=50%}
[Diferencias en diferencias]{.alert}

Comparás el antes y después de un grupo tratado contra un control que sigue una [tendencia paralela]{.alert} ([Card y Krueger, 1994](https://www.jstor.org/stable/2118030))

[Control sintético]{.alert}

Construís un "clon" del caso tratado como promedio ponderado de casos no tratados; hoy admite varios tratados ([Abadie, 2021](https://doi.org/10.1257/jel.20191450))
:::

:::{.column width=50%}
[Cuidado con el matching]{.alert}

Emparejar por características observables [no]{.alert} resuelve la selección en lo que no observás. Ya no lo tomamos como un diseño creíble por sí solo; usalo sólo si no te queda otra ([King y Nielsen, 2019](https://doi.org/10.1017/pan.2019.11))
:::
:::

:::{style="margin-top: 10px; border-left: 4px solid #2d4563; padding: 6px 18px; font-size: 22px;"}
Esto es exactamente lo de la tarde: a las 13:30 usamos las [trayectorias]{.alert} en el tiempo como contrafactual, cuando no hay ni umbral ni sorteo
:::
:::

# Apéndice: soluciones {background-color="#2d4563"}

## Solución Ejercicio 1 {#sec:sol-ej1}

:::{style="margin-top: 10px; font-size: 20px;"}
```{r}
#| label: ej1-sol
#| eval: true
#| echo: true
#| fig-height: 3.0
#| fig-width: 7
# el gráfico estándar (la versión artesanal era plot() + abline(v = 0))
rdplot(datos$matricula_primaria, datos$poblacion_c,
       x.label = "Población centrada en el umbral",
       y.label = "Matrícula primaria (chicos por aula)", title = "")

rd <- rdrobust(datos$matricula_primaria, datos$poblacion_c)
round(c(efecto = rd$coef[1], se = rd$se[1], p = rd$pv[1]), 3)
```

El efecto es [positivo]{.alert} (unos 3 chicos más por aula) y significativo: cruzar el umbral, y ganar una banca, sube la matrícula. El signo coincide con el mecanismo de más [bienes públicos]{.alert}

[[Volver al ejercicio](#sec:ej1)]{.button}
:::

## Solución Ejercicio 2 {#sec:sol-ej2}

:::{style="margin-top: 10px; font-size: 20px;"}
```{r}
#| label: ej2-sol
#| eval: true
#| echo: true
c(densidad = round(rddensity(datos$poblacion_c)$test$p_jk, 3),
  placebo  = round(rdrobust(datos$matricula_primaria, datos$poblacion_c,
                            c = 12000)$pv[1], 3),
  balance  = round(rdrobust(datos$pib_pc, datos$poblacion_c)$pv[1], 3))
```

Los tres $p$ son altos: no hay amontonamiento en el corte, no hay salto en el umbral falso, y el PIB per cápita no salta. El diseño [aguanta]{.alert}. El chequeo más grave si fallara sería el de [densidad]{.alert}: manipulación en el umbral rompe toda la comparación

:::{style="margin-top: 6px; font-size: 18px; color: #555;"}
¿Probaste el placebo en −15000? Ahí da $p \approx 0{,}03$: con muchos cortes falsos, alguno cae por azar, igual que las covariables "significativas" de la mañana. Un placebo aislado no voltea el diseño; un [patrón]{.alert} de placebos sí
:::

[[Volver al ejercicio](#sec:ej2)]{.button}
:::

## Solución Ejercicio 3 {#sec:sol-ej3}

:::{style="margin-top: 10px; font-size: 20px;"}
```{r}
#| label: ej3-sol
#| eval: true
#| echo: true
ingenua <- with(datos, mean(matricula_primaria[concejo_grande == 1]) -
                       mean(matricula_primaria[concejo_grande == 0]))

cerca <- filter(datos, abs(poblacion_c) < 8000)
local_lm <- lm_robust(matricula_primaria ~ concejo_grande * poblacion_c,
                      data = cerca)$coefficients[["concejo_grande"]]

local_rd <- rdrobust(datos$matricula_primaria, datos$poblacion_c)$coef[1]
round(c(ingenua = ingenua, local_lm = local_lm, local_rd = local_rd), 2)
```

El operador es `*`: pide las dos pendientes [más]{.alert} el salto, igual que la versión larga con `+` y `:` que usamos con la mortalidad. La ingenua arrastra la [tendencia]{.alert} (los municipios grandes ya tenían más matrícula); las dos locales la descuentan. Reportás la [local]{.alert}; la brecha es, otra vez, [sesgo de selección]{.alert}

[[Volver al ejercicio](#sec:ej3)]{.button}
:::

## Apéndice: muchos umbrales

:::{style="margin-top: 22px; font-size: 24px;"}
Nuestra simulación tiene un corte; el paper real tiene [doce]{.alert} (de 9 a 21 bancas).

- Si juntás todos los cortes [sin corregir]{.alert}, comparás municipios chicos (9-10 bancas) con grandes (20-21): aparece un desbalance de 1,63 concejales, y el estimador se sesga

- La solución del paper es [normalizar y agrupar]{.alert}: a cada municipio se lo mide como distancia a [su]{.alert} umbral más cercano, y después se apilan los doce cortes en un solo RDD ([Cattaneo, Titiunik, Vazquez-Bare y Keele, 2016](https://doi.org/10.1086/686802))

- Encima se suman [controles]{.alert}: la población de 2003, el PIB per cápita, las bancas de 2000 y una dummy de Nordeste
:::

# Nos vemos a la tarde 🍽️ {background-color="#2d4563"}

## Referencias {#sec:referencias}

:::{style="font-size: 15px;"}
:::{.columns}
:::{.column width=50%}
Abadie, A. (2021). Using synthetic controls: feasibility, data requirements, and methodological aspects. *Journal of Economic Literature* 59(2):391–425. [doi](https://doi.org/10.1257/jel.20191450)

Angrist, J. y Pischke, J.-S. (2015). *Mastering 'Metrics: The Path from Cause to Effect*. Princeton University Press. [enlace](https://press.princeton.edu/books/paperback/9780691152844/mastering-metrics)

Calonico, S., Cattaneo, M. y Titiunik, R. (2014). Robust nonparametric confidence intervals for regression-discontinuity designs. *Econometrica* 82(6):2295–2326. [doi](https://doi.org/10.3982/ECTA11757)

Card, D. y Krueger, A. (1994). Minimum wages and employment: a case study of the fast-food industry in New Jersey and Pennsylvania. *American Economic Review* 84(4):772–793. [enlace](https://www.jstor.org/stable/2118030)

Cattaneo, M., Idrobo, N. y Titiunik, R. (2020). *A Practical Introduction to Regression Discontinuity Designs: Foundations*. Cambridge University Press. [doi](https://doi.org/10.1017/9781108684606)

Cattaneo, M., Idrobo, N. y Titiunik, R. (2024). *A Practical Introduction to Regression Discontinuity Designs: Extensions*. Cambridge University Press. [doi](https://doi.org/10.1017/9781009441896)

Cattaneo, M., Jansson, M. y Ma, X. (2020). Simple local polynomial density estimators. *Journal of the American Statistical Association* 115(531):1449–1455. [doi](https://doi.org/10.1080/01621459.2019.1635480)
:::

:::{.column width=50%}
Cattaneo, M., Titiunik, R., Vazquez-Bare, G. y Keele, L. (2016). Interpreting regression discontinuity designs with multiple cutoffs. *The Journal of Politics* 78(4):1229–1248. [doi](https://doi.org/10.1086/686802)

Cunningham, S. (2021). *Causal Inference: The Mixtape*. Yale University Press. [enlace](https://mixtape.scunning.com/)

Huntington-Klein, N. (2021). *The Effect: An Introduction to Research Design and Causality*. Chapman & Hall/CRC. [enlace](https://theeffectbook.net/)

King, G. y Nielsen, R. (2019). Why propensity scores should not be used for matching. *Political Analysis* 27(4):435–454. [doi](https://doi.org/10.1017/pan.2019.11)

Lee, D. y Lemieux, T. (2010). Regression discontinuity designs in economics. *Journal of Economic Literature* 48(2):281–355. [doi](https://doi.org/10.1257/jel.48.2.281)

Londoño-Vélez, J., Rodríguez, C. y Sánchez, F. (2020). Upstream and downstream impacts of college merit-based financial aid for low-income students: Ser Pilo Paga in Colombia. *American Economic Journal: Economic Policy* 12(2):193–227. [doi](https://doi.org/10.1257/pol.20180131)

McCrary, J. (2008). Manipulation of the running variable in the regression discontinuity design: a density test. *Journal of Econometrics* 142(2):698–714. [doi](https://doi.org/10.1016/j.jeconom.2007.05.005)

Mignozzetti, U., Cepaluni, G. y Freire, D. (2025). Legislature size and welfare: evidence from Brazil. *American Journal of Political Science* 69(3):831–846. [doi](https://doi.org/10.1111/ajps.12843)
:::
:::
:::

